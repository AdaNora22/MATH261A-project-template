...
ggplot(usa_backstroke_men_swimming, aes(Year, best_time, label = Year)) +
  geom_point(size = 2) +
  ggrepel::geom_text_repel(size = 3) +   # install.packages("ggrepel") if needed
  geom_smooth(method = "lm", se = TRUE, linewidth = 0.8) +
  labs(title = "USA Trend with Year Labels", x = "Year", y = "Best USA Time (sec)") +
  theme_pubr()

# comparing USA vs another team in the same style
compare <- swimming_data |>
  filter(Stroke == "Backstroke", Gender == "Men") |>
  group_by(Team, Year) |>
  summarize(best_time = min(Results, na.rm = TRUE), .groups = "drop") |>
  filter(Team %in% c("USA","GER"))  # pick any two

ggplot(compare, aes(Year, best_time, color = Team)) +
  geom_point(alpha = 0.9) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 0.9) +
  labs(title = "Best Time per Olympics: USA vs GER (Men’s 100 Back)",
       x = "Year", y = "Best Time (sec)") +
  theme_pubr()
